...
<body>
	...
	<div class="main">
		<header class="first">
			<h2 class="airline"><%=flight.getFlightName()%>
				TICKET
		...
		<main>
			...
			<article class="content">
				<p class="s">
					PASSENGER TICKET AND BAGGAGE CHECK<br> NAME OF PASSANGER
				</p>
				<div class="name"><%=details.getName()%></div>
				<div class="loc s">
					<div class="from">
						<label>FROM :</label> <span><%=flight.getSource()%></span>
					</div>
					<div class="to">
						<label>TO :</label> <span><%=flight.getDestination()%></span>
					</div>
					<div class="board">
						<label>BOARDING :</label> <span>87866</span>
					</div>
				</div>
				<div class="box s">
					<div class="a">
						<label>FLIGHT</label> <span><label>DATE</label></span> <span><label>ZONE</label></span>
					</div>
					<div class="a">
						<span><%=flight.getFlightName()%></span> <span><%=details.getBookDate()%></span>
						<span>7263</span>
					</div>
					<br>
					<br>
					<div class="a">
						<label>GATE</label> <span><label>SEAT</label></span> <span><label>TIME</label></span>
					</div>
					<div class="a">
						<span>7263</span> <span>7263</span> <span><%=flight.getDepTime()%></span>
					</div>
				</div>
		...
		<aside>
			<div class="side">
				...
				<div>
					<div class="f">
						<label class="from">FROM: </label> <span><%=flight.getSource()%></span>
						<div class="g">
							<label>TO: </label> <span><%=flight.getDestination()%></span>
						</div>
						<div class="box2">
